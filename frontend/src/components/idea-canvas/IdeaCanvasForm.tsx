...
interface IdeaCanvasFormProps {
  onSubmit: (request: StartCanvasRequest, apiKey: string) => void;
  isStarting?: boolean;
}
...
export function IdeaCanvasForm({ onSubmit, isStarting = false }: IdeaCanvasFormProps) {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      onSubmit(request, apiKey);
    },
    [selectedTemplate, idea, provider, model, apiKey, onSubmit]
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button
        ...
        size="lg"
        className="w-full"
        disabled={isStarting || !idea.trim() || !apiKey.trim()}
      >
