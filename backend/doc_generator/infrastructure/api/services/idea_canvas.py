...
class IdeaCanvasService:
    ...
    def generate_report(self, session_id: str, api_key: str) -> dict:
...
Please generate a detailed, actionable {doc_type} document in Markdown format. Make sure to reference the specific decisions and answers provided during the exploration."""
        ...
        full_content = markdown_content + footer

        # Generate PDF
        pdf_base64 = self._generate_pdf_from_markdown(title, full_content)
    ...
    def _generate_pdf_from_markdown(self, title: str, markdown_content: str) -> str:
